...
//   // Delete a comment (only by the comment author or post owner)
//   public async destroy({ request, params, response }: HttpContextContract) {
//     try {
//       const userId = request.encryptedCookie('user_id')
//       if (!userId) {
...
//       const canDelete = comment.profileId === userId || comment.post.profileId === userId
//       if (!canDelete) {
//         return response.status(403).json({ message: 'You can only delete your own comments or comments on your posts' })
//       }
